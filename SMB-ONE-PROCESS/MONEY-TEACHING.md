...
- ✅ Start every initial backtest with a threshold at least 2 percentage points lower than your initial assumption to test a broader range
...
- ✅ Run your initial backtest with no stop and no target, then use MF and MAE analysis to identify exit levels
- ✅ Apply only selected rules to keep strategies simple
...
- ❌ Do not fear curve fitting to the point of inaction; start with a number of trades and let tools identify rules that meet a threshold
- ❌ Do not take partial profits and move stops to break even without running backtests to compare outcomes
- ❌ Do not abandon a strategy during a drawdown within your tolerance if you did not build and understand it yourself
- ❌ Do not hoard ideas; sharing strategy concepts generates reciprocal collaboration

## Keep In Mind

- The signal is not the most important factor compared to the optimization process; the optimization process determines your returns.
...
- Your column library is a reusable set of indicators that becomes more effective as you develop more strategies.
- Your highest-performing strategy has not been identified yet through your current process; your process determines whether you identify it.

## Security & Safety Notes

- Never share proprietary backtest CSV files or column libraries publicly.
- Be cautious about who you collaborate with; share ideas in relationships where you have established trust rather than open forums.
...
- **Problem:** You get a profitable backtest on the first try and start live trading immediately.
  **Solution:** Always test a broader range first. An initial backtest that meets your performance criteria with too few trades is overfitted to the historical dataset. Widen your thresholds, add columns, and confirm the rule holds across a number of trades before risking capital.
...
-   **Problem:** You add several rules to filter trades and end up curve fitting.
  **Solution:** Let the optimizer identify the rules. Fewer rules produce more robust strategies than adding more rules.
...
- **Problem:** You abandon a strategy during a drawdown because you do not understand how it was built.
  **Solution:** Only trade strategies you built or fully rebuilt from original data. If you inherited it, rebuild it from scratch so you have the discipline to continue trading through drawdowns.
