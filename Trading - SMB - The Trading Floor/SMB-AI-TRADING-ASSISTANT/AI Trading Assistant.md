...
- When you use TraderView, which tracks trade performance statistics, tags trades, and provides profit and loss (P&L) breakdowns and win-loss ratios, but want stats and tabs for the specific setups and patterns you track that TraderView does not cover
- When you want to automate your end-of-day trade review and journaling routine
- When you want Claude Code to analyze your trade write-ups and flag recurring mistakes or tendencies
...
Open Claude Code and switch to Plan Mode by toggling off "Accept Edits." In Plan Mode, Claude Code does not modify any files—it only asks questions and helps you design a concrete implementation plan. Craft your first prompt to describe the dashboard you want. For example:
...
Claude Code will then ask questions in batches of three or four. Answer each batch, and after each round Claude Code updates the plan document. Continue answering questions until Claude Code signals it has enough information. Typical questions include:

- What platform should the dashboard live on? (Choose: local HTML single-page app you open in a web browser)
- Where should dashboard data live? (Choose: daily entries, notes, tags—human-readable format)
- How does data get into the dashboard each day? (Manual entry, auto-pull script, or decide later)
...
Once all questions are answered, Claude Code will generate a complete .md implementation plan document. Save that file.

### Step 2: Build Mode — Build the Skeleton

Start a new Claude Code session (or stay in the Plan Mode session from Step 1), make sure you are NOT in Plan Mode (toggle "Accept Edits" ON), and paste your saved .md implementation plan into the session. Tell Claude Code: "Here's my implementation plan for a trading dashboard that I'm trying to build. I want you to build the skeleton of this."

Claude Code will then build the full HTML dashboard skeleton. Once it finishes, you will get a link to open the dashboard in a web browser. No further coding is required on your part during this step—Claude Code builds the HTML dashboard skeleton and provides you a browser link.

### Step 3: Personalize the Dashboard

Open the dashboard in a web browser using the link Claude Code provided. Review what was built. Now go back to Claude Code (still in Build Mode / Accept Edits ON) and start making one-off, personalized changes. Examples of personalization include:
...
Once the dashboard is built and personalized, make the workflow repeatable by setting up a Claude Code Routine. Tell Claude Code you want a routine scheduled for after the regular U.S. equities market session ends (Eastern Time), at a specific time each day (e.g., 4:15 PM ET or 4:30 PM ET). Define what questions Claude Code should ask you during that routine to update the dashboard. For example, Claude Code can prompt you:
...
Claude Code will then auto-populate the dashboard based on your answers. You can also set routines for before the regular U.S. equities market session opens (pre-market, before 9:30 AM ET) or for weekly review intervals. The key is that once the routine is set up, you only need to respond to Claude Code's prompts—Claude Code handles the prompting and the dashboard updates automatically.

## Examples

### Example 1: A Performance-Based Trading Dashboard With Coach Layer

Tim, co-host of the Trading Floor podcast (available on Spotify), built a dashboard that tracks every trade taken in 2026, splits results by grade (A+, A, A-, B), and shows cumulative P&L. He added a "Tendencies" page that displays recurring patterns like "no man's land sizing" (mentioned five times in trade write-ups) and "end-of-month B cap" (where he catches himself over-grading B trades). He also added a performance coach layer: every time he inputs a new trade write-up, Claude Code reviews his stats, tendencies, and journal entries, and produces a coach's note like, "Those were two no man's land sizing trades today—something we really want to keep an eye on."

### Example 2: A Dynamic Resource / Setup Grading Dashboard

Garrett, co-host of the Trading Floor podcast (available on Spotify), described building a dashboard focused on his specific plays: tracking catalysts, setups that are working vs. not working, and grading setups. He wants Claude Code to understand his checks in favor and variables for each play, then help him grade new setups by comparing them against his historical trade data. For example, if a breakout trend trade closes right at the breakout level, Claude Code could pull up all similar past setups and show: "You've taken 20 breakout trend trades in the past year. 70% closed above the level and worked. None closed below and worked. Consider lowering the grade or getting flat."
...
- Attach template images (e.g., screenshots of TraderView pages you like) to your initial prompt so Claude Code has a visual reference for the end goal
...
- Be specific in your trade write-ups and journal entries—the more context you give Claude Code, the more accurately it can detect patterns in your trading behavior and include them in a coach's note
...
- The quality of what Claude Code can detect in your trading depends on the detail in your write-ups. Trade entries that include executions, allocation grades, accuracy assessments, mistakes, and reasoning give Claude Code more material to work with; entries with minimal detail limit what it can surface.
- Use Claude Code as an information organizer and pattern detector, not as a decision-maker for entries or exits. Write a clear instruction defining Claude Code's role so it summarizes past performance rather than instructing you to trade.
...
- The dashboard is a local HTML file that opens in a web browser. It lives on your machine and is not hosted online, so your trade data stays private.
...
- Review any code Claude Code generates before using it, especially if you modify the dashboard to connect to external APIs or brokerages.
...
- **Problem:** Jumping straight into Build Mode without a plan, leading to messy code and constant rework.
  **Solution:** Complete Step 1 (Plan Mode) first. Let Claude Code write the full .md plan, review it, and only then switch to Build Mode.
- **Problem:** Overloading the initial prompt with too many requirements at once, causing Claude Code to miss key details.
...
- **Problem:** Forgetting to backfill old trades, so the dashboard has no historical context for pattern detection.
  **Solution:** Enter your recent trades after building the skeleton so Claude Code has data to analyze.
- **Problem:** Letting Claude Code make entry or exit decisions instead of just providing information.
   **Solution:** Write a clear instruction framing Claude Code's role as a coach and organizer. Do not ask it to tell you whether to enter or exit a trade—ask it to summarize your past performance in similar setups.